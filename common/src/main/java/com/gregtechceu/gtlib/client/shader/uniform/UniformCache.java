...
//				GTLib.LOGGER.error("can't find uniform with name {}", location);
//				var shaders = new int[2];
//				GL20.glGetAttachedShaders(programId, null, shaders);
//				GTLib.LOGGER.error("attached shader source {}", GL20.glGetShaderSource(shaders[0]));
//				GTLib.LOGGER.error("attached shader source {}", GL20.glGetShaderSource(shaders[1]));
			}
			return;
		}
		boolean update = true;
