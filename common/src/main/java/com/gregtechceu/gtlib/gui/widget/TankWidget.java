...
import com.gregtechceu.gtlib.side.fluid.*;
...
public class TankWidget extends Widget implements IRecipeIngredientSlot, IConfigurableWidget {
    ...
    public TankWidget() {
        ...
        setBackground(new ResourceTexture("gtlib:textures/gui/fluid_slot.png"));
    ...
    public void drawInForeground(@Nonnull PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (drawHoverTips && isMouseOverElement(mouseX, mouseY)) {
            ...
            if (lastFluidInTank != null && !lastFluidInTank.isEmpty()) {
                ...
                tooltips.add(Component.translatable("gtlib.fluid.amount", lastFluidInTank.getAmount(), lastTankCapacity));
                tooltips.add(Component.translatable("gtlib.fluid.temperature", FluidHelper.getTemperature(lastFluidInTank)));
                tooltips.add(Component.translatable(FluidHelper.isLighterThanAir(lastFluidInTank) ? "gtlib.fluid.state_gas" : "gtlib.fluid.state_liquid"));
            } else {
                tooltips.add(Component.translatable("gtlib.fluid.empty"));
                tooltips.add(Component.translatable("gtlib.fluid.amount", 0, lastTankCapacity));
            }
            if (gui != null) {
    ...
    public void buildConfigurator(ConfiguratorGroup father) {
        ...
        father.addConfigurators(new WrapperConfigurator("gtlib.gui.editor.group.preview", new TankWidget() {
            @Override
